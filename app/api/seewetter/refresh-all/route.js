...
function isVercelCron(req) {
  ...
  if (
    process.env.SEEWETTER_REFRESH_TOKEN &&
    token === process.env.SEEWETTER_REFRESH_TOKEN
  ) {
    return true;
  }
...
const MARKERS = {
  ...
  en: {
    ...
    outlook_12h: [
      ...
      'Weather outlook for the next 12 hours',
    ],
  },
  it: {
...
function extractSectionsFromHtml(html, lang) {
  ...
  const root = $('#primary .glavni__content').first().length
    ? $('#primary .glavni__content').first()
    : $('#primary').first().length
      ? $('#primary').first()
  ...
  const title =
    ...
    titleFromH1 ||
    titleFromTitle ||
    (bodyText.toLowerCase().includes('seewetterbericht')
  ...
  if (rawSections.length < 2 || looksLikePortal) {
    ...
    const w = extractBlockByMarkers(bodyText, m.warning, [
      ...m.synopsis,
      ...m.forecast_12h,
      ...m.outlook_12h,
    ]);
    const s = extractBlockByMarkers(bodyText, m.synopsis, [
      ...m.forecast_12h,
      ...m.outlook_12h,
    ]);
...
function mapToBlocks(rawSections) {
  ...
  for (const sec of rawSections) {
    ...
    if (
      !blocks.warning &&
      (h.includes('warn') || h.includes('warning') || h.includes('avvert') || h.includes('upozor'))
    ) {
  ...
  if (nonNullCount < 2 && rawSections.length >= 2) {
    const pick = (i, key) => {
      if (!blocks[key] && rawSections[i]) {
        blocks[key] = { label: rawSections[i].label, text: rawSections[i].text };
      }
    };
    pick(0, 'warning');
    pick(1, 'synopsis');
...
async function refreshOneLang(lang) {
  ...
  console.log('HTML RECEIVED', lang, html.length);
...
export async function GET(req) {
  ...
  for (const lang of langs) {
    try {
      console.log('REFRESH START', lang);
      const r = await refreshOneLang(lang);
      console.log('REFRESH SUCCESS', lang, r);
      results.push(r);
    } catch (e) {
      console.error('REFRESH ERROR', lang, e);
