...
function pickKey() {
  // Server-only keys (keine NEXT_PUBLIC Keys in einer Server-Route!)
  return (
...
export async function GET(req) {
  try {
    ...
    // akzeptiere beide Varianten vom Client:
    const ref =
    ...
    // Größenparameter: wir unterstützen maxwidth/maxheight
    const mw = url.searchParams.get("maxwidth");
    const mh = url.searchParams.get("maxheight");

    // Google Places Photo (Legacy) verlangt mind. maxwidth ODER maxheight
    ...
    // WICHTIG: Legacy endpoint erwartet "photoreference" (ohne underscore)
    qs.set("photoreference", ref);
    qs.set("key", key);

    const gUrl =
      "https://maps.googleapis.com/maps/api/place/photo?" + qs.toString();
